<script lang="ts">
	...
	import IconEmpty from '$lib/components/icons/IconEmpty.svelte';
	import IconPencil from '$lib/components/icons/IconPencil.svelte';
	import IconTrash from '$lib/components/icons/IconTrash.svelte';
	import IconVerticalDots from '$lib/components/icons/IconVerticalDots.svelte';
	import debounce from 'debounce';
	import { createEventDispatcher, onDestroy } from 'svelte';
	import { fade } from 'svelte/transition';
	import BusyOverlay from './BusyOverlay.svelte';

	type T = { id: string; updatedAt: Date };
				...
				{#if items.length}
					{#each items as item, index (item.id)}
						<tr>
							...
							{/each}
							<td class="align-right nowrap">{formatDistanceToNow(new Date(item.updatedAt))}</td>
...
<style>
	...
	.icon-button.row:not(:first-child) {
		margin-left: 0.25em;
	}
